...
def search(index, item, api_key):
    ...
    try:
        ...
        return {"index": index, "error": {"status": error.code, "message": error.read(1000).decode("utf-8", "replace")}}
...
def main():
    ...
    if not api_key:
        sys.exit("PARALLEL_AI_API_KEY is not set")
    try:
        with open(sys.argv[1], encoding="utf-8") as file:
            searches = json.load(file)["searches"]
        if not isinstance(searches, list) or len(searches) < 2:
            raise ValueError("searches must contain at least two requests")
        for item in searches:
            if not isinstance(item.get("objective"), str) or not isinstance(item.get("search_queries"), list) or not item["search_queries"]:
